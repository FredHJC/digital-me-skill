...
def main() -> None:
    ...
    if not content:
        print("⚠️  未获取到内容", file=sys.stderr)
        sys.exit(1)

    # PII 脱敏 → JSON 输出
    scrubbed_content, stats = scrub(content, use_llm_fallback=args.use_llm_fallback)
    chunks = [{"id": 0, "text": scrubbed_content, "metadata": {"source": source_name}}]
    out_path = write_ingestion_json(
        chunks=chunks,
        source_type="feishu_mcp",
    ...
    print(f"✅ 已保存（已脱敏）→ {out_path}", file=sys.stderr)
