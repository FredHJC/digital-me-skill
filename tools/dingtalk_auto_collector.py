...
def collect_all(
    ...
    try:
        ...
        if doc_content and len(doc_content.strip()) > 20:
            ...
            doc_path = write_ingestion_json(
                chunks=doc_chunks,
                source_type="dingtalk_api",
                context_label=context_label,
                twin_slug=twin_slug,
                source_file=f"dingtalk_docs_{name}",
                scrub_stats=doc_stats,
                base_dir=base_dir,
            )
            results["docs"] = str(doc_path)
            print(f"  ✅ 文档（已脱敏）→ {doc_path}", file=sys.stderr)
    except Exception as e:
        print(f"  ⚠️  文档采集失败：{e}", file=sys.stderr)

    # Step 3: 多维表格 → PII 脱敏 → JSON 输出
    print(f"\n📊 采集多维表格 ...", file=sys.stderr)
    try:
        bitable_content = collect_bitables(user, config)
        if bitable_content and len(bitable_content.strip()) > 20:
            scrubbed_bt, bt_stats = scrub(bitable_content, use_llm_fallback=use_llm_fallback)
            bt_chunks = [{"id": 0, "text": scrubbed_bt, "metadata": {"source": "bitables"}}]
            bt_path = write_ingestion_json(
                chunks=bt_chunks,
                source_type="dingtalk_api_bitable",
                context_label=context_label,
                twin_slug=twin_slug,
                source_file=f"dingtalk_bitables_{name}",
                scrub_stats=bt_stats,
                base_dir=base_dir,
            )
            results["bitables"] = str(bt_path)
            print(f"  ✅ 多维表格（已脱敏）→ {bt_path}", file=sys.stderr)
    except Exception as e:
        print(f"  ⚠️  多维表格采集失败：{e}", file=sys.stderr)

    # Step 4: 消息记录（浏览器方案）→ PII 脱敏 → JSON 输出
    if not skip_messages:
        ...
        try:
            ...
            if msg_content and len(msg_content.strip()) > 20:
                scrubbed_msg, msg_stats = scrub(msg_content, use_llm_fallback=use_llm_fallback)
                msg_chunks = [{"id": 0, "text": scrubbed_msg, "metadata": {"source": "messages"}}]
                msg_path = write_ingestion_json(
                    chunks=msg_chunks,
                    source_type="dingtalk_api",
                    context_label=context_label,
                    twin_slug=twin_slug,
                    source_file=f"dingtalk_messages_{name}",
                    scrub_stats=msg_stats,
                    base_dir=base_dir,
                )
                results["messages"] = str(msg_path)
                print(f"  ✅ 消息记录（已脱敏）→ {msg_path}", file=sys.stderr)
        except Exception as e:
    ...
    print(f"\n✅ 采集完成 → twins/{twin_slug}/knowledge/{context_label}/", file=sys.stderr)
