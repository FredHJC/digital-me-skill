...
def collect_all(
    ...
    """采集某用户的所有可用数据，输出脱敏 JSON 到 twins/{slug}/knowledge/{context}/"""
    results = {}
    ...
    # Step 2: 采集消息记录 → PII 脱敏 → JSON 输出
    print(f"\n📨 采集消息记录（上限 {msg_limit} 条）...", file=sys.stderr)
    try:
        raw_messages = _collect_raw_messages(user, msg_limit, config)
        if raw_messages:
            chunks: list[dict] = []
            combined_stats: dict[str, int] = {}
            for i, msg in enumerate(raw_messages):
                text = msg.get("content", msg.get("text", ""))
                scrubbed, msg_stats = scrub(text, use_llm_fallback=use_llm_fallback)
                chunks.append({
                    "id": i,
                    "text": scrubbed,
                    "metadata": {
            ...
            out_path = write_ingestion_json(
                chunks=chunks,
                source_type="feishu_api",
                context_label=context_label,
                twin_slug=twin_slug,
                source_file=f"feishu_{name}",
                scrub_stats=combined_stats,
                base_dir=base_dir,
            )
            results["messages"] = str(out_path)
            print(f"  ✅ 消息记录（已脱敏）→ {out_path}", file=sys.stderr)
        else:
            print(f"  ⚠️  未采集到消息记录", file=sys.stderr)
    except Exception as e:
        print(f"  ⚠️  消息采集失败：{e}", file=sys.stderr)

    # Step 3: 采集文档 → PII 脱敏 → JSON 输出
    print(f"\n📄 采集文档（上限 {doc_limit} 篇）...", file=sys.stderr)
    try:
        doc_content = collect_docs(user, doc_limit, config)
        if doc_content and len(doc_content.strip()) > 20:
            scrubbed_doc, doc_stats = scrub(doc_content, use_llm_fallback=use_llm_fallback)
            doc_chunks = [{"id": 0, "text": scrubbed_doc, "metadata": {"source": "docs"}}]
            doc_path = write_ingestion_json(
                chunks=doc_chunks,
                source_type="feishu_api_docs",
                context_label=context_label,
                twin_slug=twin_slug,
                source_file=f"feishu_docs_{name}",
                scrub_stats=doc_stats,
                base_dir=base_dir,
            )
            results["docs"] = str(doc_path)
            print(f"  ✅ 文档内容（已脱敏）→ {doc_path}", file=sys.stderr)
    except Exception as e:
    ...
    summary_dir = base_dir / twin_slug / "knowledge" / context_label
    summary_dir.mkdir(parents=True, exist_ok=True)
    (summary_dir / "collection_summary.json").write_text(
        json.dumps(summary, ensure_ascii=False, indent=2)
    )

    print(f"\n✅ 采集完成，输出到 twins/{twin_slug}/knowledge/{context_label}/", file=sys.stderr)
    return results


def _collect_raw_messages(user: dict, msg_limit: int, config: dict) -> list:
    """采集消息记录，返回原始消息列表（供 collect_all 脱敏处理）"""
    user_open_id = user.get("open_id") or user.get("user_id", "")
    all_messages = []
...
def main() -> None:
    ...
    parser.add_argument("--output-dir", default=None, help="（已废弃，保留向后兼容）输出目录")
    ...
    # 验证关系标签
    context_label = validate_context_label(args.context)
    ...
    if args.open_id:
        ...
        raw_messages = _collect_raw_messages(user, args.msg_limit, config)
        if raw_messages:
            chunks: list[dict] = []
            combined_stats: dict[str, int] = {}
            for i, msg in enumerate(raw_messages):
                text = msg.get("content", msg.get("text", ""))
                scrubbed, msg_stats = scrub(text, use_llm_fallback=args.use_llm_fallback)
                chunks.append({
                    "id": i,
                    "text": scrubbed,
                    "metadata": {
            ...
            out_path = write_ingestion_json(
                ...
                context_label=context_label,
                twin_slug=args.slug,
                source_file=f"feishu_{args.open_id}",
                scrub_stats=combined_stats,
                base_dir=base_dir,
            )
            print(f"  ✅ 消息记录（已脱敏）→ {out_path}", file=sys.stderr)
