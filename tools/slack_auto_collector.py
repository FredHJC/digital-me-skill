...
def collect_all(
    ...
    """采集某用户的所有 Slack 数据，输出脱敏 JSON 到 twins/{slug}/knowledge/{context}/"""
    results: dict = {}
    ...
    # Step 3: 采集消息 → PII 脱敏 → JSON 输出
    print(f"\n📨 采集消息记录（上限 {msg_limit} 条）...", file=sys.stderr)
    try:
        raw_messages = _collect_raw_messages(user, channels, msg_limit, client)
        if raw_messages:
            chunks: list[dict] = []
            ...
            for i, msg in enumerate(raw_messages):
                ...
                chunks.append({
                    ...
                    "metadata": {
                        ...
                        "is_thread_starter": msg.get("is_thread_starter", False),
                    },
                })
            ...
            msg_path = write_ingestion_json(
                ...
                twin_slug=twin_slug,
                source_file=f"slack_{real_name}",
            ...
            print(f"  ⚠️  未采集到消息", file=sys.stderr)
    ...
    summary_path = summary_dir / "collection_summary.json"
...
def _collect_raw_messages(
    ...
    """从所有频道采集原始消息列表（供 collect_all 脱敏处理）"""
    user_id = user["id"]
    all_messages: list = []
    per_channel_limit = max(100, msg_limit // len(channels)) if channels else msg_limit
...
示例：
  ...
  # 采集数据
  python3 slack_auto_collector.py --name "张三" --context coworker --slug my-twin
  python3 slack_auto_collector.py --name "john.doe" --context friend --slug my-twin --msg-limit 500
        """,
    )
    parser.add_argument("--setup", action="store_true", help="初始化配置（Bot Token）")
    parser.add_argument("--name", help="用户姓名或 Slack 用户名")
    parser.add_argument(
        "--output-dir",
        default=None,
        help="（已废弃，保留向后兼容）输出目录",
    )
    parser.add_argument(
        "--msg-limit",
        type=int,
