...
def main():
    ...
    try:
        if suffix == ".eml":
            parsed_emails = parse_eml_file(str(file_path), args.target)
        elif suffix == ".mbox":
            parsed_emails = parse_mbox_file(str(file_path), args.target)
        else:
            parsed_emails = parse_txt_file(str(file_path), args.target)
    except Exception as e:
        print(f"错误：解析邮件文件失败：{e}", file=sys.stderr)
        sys.exit(1)
    ...
    # PII 脱敏 → JSON 输出（每封邮件一个 chunk）
    chunks: list[dict] = []
    combined_stats: dict[str, int] = {}
    for i, em in enumerate(parsed_emails):
        body = em.get("body", "")
        scrubbed_body, em_stats = scrub(body, use_llm_fallback=args.use_llm_fallback)
        chunks.append({
            "id": i,
            "text": scrubbed_body,
            "metadata": {
    ...
    print(f"已输出 {len(parsed_emails)} 封邮件（已脱敏）→ {out_path}")
